...
void node_dynnamics(int U,int *matrix, int *list_states, int N, int node){
    ...
    if (number_neighboors>0){
        ...
        if( abs((*(list_states+node)-*(list_states+index_neighboors[node_2])))<=U ){
...
void evolution(float Pc, float Pr, int U,float d, float r, int *matrix, int *list_states,int N,int n_evol){
...
void main(){
    ...
    for(int counter_U=0;counter_U<=size;counter_U++){
        printf("%d\n",U);
        for(int counter_Pr=0;counter_Pr<size;counter_Pr++){
            for (int run=0;run<runs;run++){
                    char info_1[20];
                    sprintf(info_1,"U_%d_Pr_%d_run%d.dat",U,counter_Pr,run);
                    int *matrix;
                    int *list_states;
                    float p=(float)k/(N-1);
