...
int rewire_2(float d, float r,int *matrix, double *list_states, int N, int node,void *ptr_rng_r){
...
int rewire(float d, float r,int *matrix, double *list_states, int N, int node,void *ptr_rng_r){
    gsl_rng *rng_r=(gsl_rng*)ptr_rng_r;
    ...
    //d=(float)nodes_same_state/(nodes_different_state+nodes_same_state);
...
void evolution(float Pc, float Pr, float U,float d, float r, int *matrix, double *list_states,int N,int n_evol,void *ptr_rng_r){
    ...
    for (int t_step=0;t_step<n_evol;t_step++){
        ...
        if (n_neighboors>0){
            ...
            int temp_rewire=0;
            if (temp_Pr<=Pr){
            temp_rewire=rewire(d,r, matrix,list_states, N,node, rng_r);
            t_step+=temp_rewire;
